...
export class VerbQuestScene extends Phaser.Scene {
  ...
  private resetProjectile(){if(this.projectile)this.projectile.destroy();this.aiming=false;this.flight=false;this.pull={x:0,y:0,d:0};this.resetPouch();this.projectile=this.matter.add.sprite(this.rest.x,this.rest.y,'orb',undefined,{frictionAir:.012,restitution:.55,density:.002}).setCircle(19).setDisplaySize(44,44).setTint(0xffe55d).setDepth(15);this.projectile.setStatic(true);}
  pointer(x:number,y:number,down:boolean,release=false,hand=false){if(!this.projectile||this.flight)return;if(hand&&this.cursor){this.cursor.setPosition(x,y).setVisible(true);}if(down){if(!this.aiming&&(hand||Phaser.Math.Distance.Between(x,y,this.rest.x,this.rest.y)<80))this.aiming=true;if(this.aiming){const p=pullLimit(x-this.rest.x,y-this.rest.y,this.maxPull);this.pull=p;this.projectile.setPosition(this.rest.x+p.x,this.rest.y+p.y);this.pouch.setPosition(this.rest.x+p.x,this.rest.y+p.y);this.drawBands(this.pouch.x,this.pouch.y);this.eventsOut.aim(p.d/this.maxPull);this.drawTrajectory();}} else if(release&&this.aiming)this.launch();}
