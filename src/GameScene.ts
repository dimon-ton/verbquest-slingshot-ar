...
export class VerbQuestScene extends Phaser.Scene {
  ...
  private layout(){const w=this.scale.width,h=this.scale.height;this.rest={x:w/2,y:h-78};this.resetPouch();}
  ...
  pointer(x:number,y:number,down:boolean,release=false,hand=false){if(!this.projectile||this.flight)return;if(hand&&this.cursor){this.cursor.setPosition(x,y).setVisible(true);}if(down){if(!this.aiming&&Phaser.Math.Distance.Between(x,y,this.rest.x,this.rest.y)<80)this.aiming=true;if(this.aiming){const p=pullLimit(x-this.rest.x,y-this.rest.y);this.pull=p;this.projectile.setPosition(this.rest.x+p.x,this.rest.y+p.y);this.pouch.setPosition(this.rest.x+p.x,this.rest.y+p.y);this.drawBands(this.pouch.x,this.pouch.y);this.eventsOut.aim(p.d/180);this.drawTrajectory();}} else if(release&&this.aiming)this.launch();}
  ...
  private drawTrajectory(){this.orbit.clear();const v=launchVelocity(this.pull),start=this.projectile!;this.orbit.fillStyle(0xa9fff8,.85);for(let t=.08;t<1.15;t+=.08){const x=start.x+v.x*60*t;const y=start.y+v.y*60*t+.5*1.35*60*t*t;if(y>this.scale.height-15)break;this.orbit.fillCircle(x,y,3);}}
  private launch(){const p=this.projectile;if(!p)return;this.aiming=false;this.flight=true;this.orbit.clear();this.pouch.setPosition(this.rest.x,this.rest.y);this.drawBands(this.rest.x,this.rest.y);p.setStatic(false);const v=launchVelocity(this.pull);p.setVelocity(v.x,v.y);this.eventsOut.aim(0);}
